/*
Lifted from https://github.com/vivien/i3blocks-contrib/tree/master/bandwidth2#bandwidth2
*/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
void display(int const unit, double b, int const warning, int const critical, bool show_diver)
{
  ...
  if (b < 1024) {
    printf("%4.0lf  %c/s", b, unit);
  } else if (b < 1024 * 1024) {
    printf("%4.0lf K%c/s", b / 1024, unit);
  } else if (b < 1024 * 1024 * 1024) {
    printf("%4.0lf M%c/s", b / (1024 * 1024), unit);
  } else {
    printf("%4.0lf G%c/s", b / (1024 * 1024 * 1024), unit);
  }
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    display(unit, rx, warningrx, criticalrx, true);
    printf(" ");
    display(unit, tx, warningtx, criticaltx, false);
    fflush(stdout);
    s_old = s;
